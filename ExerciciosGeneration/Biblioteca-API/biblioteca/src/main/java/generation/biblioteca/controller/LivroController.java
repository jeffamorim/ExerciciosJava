package generation.biblioteca.controller;
...
import generation.biblioteca.repository.LivroRepository;
...
public class LivroController {
	...
	public ResponseEntity<Livro> GetById(@PathVariable long isbn){
		return repository.findById(isbn).map( resp -> ResponseEntity.ok(resp)).orElse(ResponseEntity.notFound().build());
	}
